...
- [x] Cloudflare Pages project `lexly-heyitsmejosh` is NOT git-connected (`Git Provider: No`), so `git push` does not deploy the site — it silently went 2 weeks stale and caused both bugs above — 2026-08-04: added `scripts/deploy.sh` (stages the publishable paths into a `mktemp -d` then `npx wrangler pages deploy --project-name lexly-heyitsmejosh --branch main`; never the repo root, `ios/build*` exceeds the Pages file limit). Ran it end to end (60 files + Functions bundle, deployment `61fea038`) and verified live: 0 hits for fraunces/googleapis, `/assets/icon.svg` matches source. Documented under "Deploying" in CLAUDE.md so a plain push is never mistaken for a deploy again. Left the second, unused `lexly` Pages project alone.
