...
- [x] **Listing de-fingerprinted 2026-08-28.** Rewrote the App Store description to drop the portfolio-wide formula (the "X is a Y app that…" opener, the `Features:` + bullet block, the "No subscription. No ads." closer), swapped the generic single-word keyword CSV for Lexly-specific phrases, and pointed the support URL at `lexly.heyitsmejosh.com/support` instead of the marketing homepage. Verified applied by re-pulling from ASC.
...
- [x] **Resolution Center reply FILED 2026-08-28 08:51 PT**, posted as "Joshua Trommel" on thread `1d490667`. Asks Apple to name the comparison app and cites the four prior approvals, the zero shared Swift files, and the rewritten listing. Awaiting response.
...
- [x] **SECURITY: leaked Resend API key — closed 2026-08-28.** Key `re_DdSyTbZa…` is **already dead**: `GET https://api.resend.com/api-keys` with it returns 400 "API key is invalid", so it is not exploitable. It leaked via Xcode build intermediates (`ios/build-shots/Build/Intermediates.noindex/…`) committed by mistake in `2182d3f`, removed in `71cfaf1`; `ios/build-shots/` is gitignored (`.gitignore:21`) so it cannot recur. Nothing in the repo references RESEND. GitHub secret-scanning alert #1 resolved as `revoked` via `gh api`.
...
- [x] Removed the duplicated "Now on the App Store · free to play" hero pill from `index.html` (the `apple-itunes-app` Smart App Banner on line 9 already says it); dropped the now-dead `.status-pill`/`.status-dot` CSS with it — 6 lines deleted, no other users.
- [x] Added a boot splash to `app/index.html`: inline `<style>`+markup so it paints before `css/lingo.css` loads, app icon + spinner, dismissed on `window.load` with an 8s failsafe so a stalled asset can never leave it covering the app. Light/dark via `data-theme`, honors `prefers-reduced-motion`.
