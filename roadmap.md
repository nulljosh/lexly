...
- [x] Footer text spacing needs improvement — app footer (`app/index.html:173`) was cramped: `padding:1.5rem`, `font-size:0.75rem`, no top margin, no gap. Now matches the landing footer's rhythm: `margin:3rem auto 0`, `padding:2rem 1.5rem`, `font-size:0.8125rem`, `line-height:1.6`, `flex-wrap:wrap`, `gap:1rem`.
...
- [x] Top subject nav scrolls vertically — fixed 2026-07-25. Container is `.category-tabs` (`app/index.html:102`, `css/lingo.css:219`). Root cause: `overflow-x: auto` forces the other axis to compute from `visible` to `auto`, and `.category-tab`'s `margin-bottom: -2px` (used to overlap the container's `border-bottom`) pushed 2px past the padding box → 2px of vertical overflow made it vertically scrollable. Fix: swapped the container's `border-bottom` for `box-shadow: inset 0 -2px 0 var(--border)` (paints inside the padding box, so the tab's own 2px `border-bottom` aligns with it without a negative margin), dropped `margin-bottom: -2px`, added `overflow-y: hidden`. Active-tab underline alignment preserved.
...
- [x] School dashboard link broken — fixed 2026-07-25. Same root cause as the 2026-07-21 masterclass-link fix: relative URLs resolving wrong from the `/app/` route. The profile panel's "School dashboard" button (`js/lingo-app.js:619`) used `href = 'school/index.html'`, which resolves to `/app/school/index.html` — a path that doesn't exist and never matches `middleware.js`'s `/school/:path*` matcher, so it 404s out to the landing page. Changed to `/school/index.html`. Grepped the rest of `js/lingo-app.js` + `js/games.js` for sibling relative paths — this was the last one (course packs already use absolute `/content/courses/...`).
