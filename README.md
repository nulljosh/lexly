...
- [ ] Native iOS/macOS visual parity pass: `ios/` is a separate native SwiftUI codebase (not a WebView), so the new web landing/vibe styling doesn't carry over automatically. Needs its own scoped pass: update SwiftUI views to match, rebuild, regenerate fastlane screenshots, then resubmit as a new version (would supersede the 1.1.0 currently WAITING_FOR_REVIEW — confirm with Josh before bumping).
